...
float bdepth(vec2 coords) //blurring depth
{
	...
	for( int i=0; i<9; i++ )
	{
		float tmp = texture2D(s_texture2, coords + offset[i]).r;
		d += tmp * kernel[i];
	}
...
vec3 color(vec2 coords,float blur) //processing the sample
{
	...
	col.r = texture2D(s_texture,coords + vec2(0.0,1.0)*texel*fringe*blur).r;
	col.g = texture2D(s_texture,coords + vec2(-0.866,-0.5)*texel*fringe*blur).g;
	col.b = texture2D(s_texture,coords + vec2(0.866,-0.5)*texel*fringe*blur).b;
...
void main() 
{
	...
	float depth = linearize(texture2D(s_texture2,texCoord).x);
	...
	if (autofocus)
	{
		fDepth = linearize(texture2D(s_texture2,focus).x);
	}
	...
	{
		col = texture2D(s_texture, texCoord).rgb;
	}
